...
Claude Code에 **Skill 기반의 동작 디시플린**을 입히는 확장 모음. obra(Jesse Vincent) 가 만들어 공개한 것이 베이스이며 사용자/팀별로 커스터마이징해서 씁니다.
...
스킬은 Claude Code의 표준 위치에 두면 자동 인식됩니다.

```
~/.claude/skills/<skill-name>/SKILL.md
```

또는 플러그인 형태로 배포된 패키지 설치:

```bash
# 예시 (실제 패키지명/소스는 확인 필요)
git clone https://github.com/obra/superpowers ~/.claude/plugins/superpowers
```

설치 후 `/skills` 또는 사용자 확인용 슬래시 커맨드로 활성 스킬 확인.
...
- 베이스: https://github.com/obra/superpowers (정확한 위치 확인 필요)
- 스킬 작성법: Claude Code 내부의 `superpowers:writing-skills` 스킬
